...
class Crc16ARC {
    ...
    public function ComputeCrc($data){

        $CRC_16_ARC_ = new CrcParams();
        $CRC_16_ARC_->Init = 0x0;
        $CRC_16_ARC_->XorOut = 0x0;
        $CRC_16_ARC_->RefOut = TRUE;

        $CRC_16_ARC_->Poly = 0x8005;
        $CRC_16_ARC_->RefIn = TRUE;
        $CRC_16_ARC_->Check = 0xBB3D;
        $CRC_16_ARC_->Array = array(0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0,
        ...
        for ($offset = 0; $offset < $length; $offset++) {
            $result ^= bindec(strrev(sprintf( "%08d", decbin(ord($data[$offset]))))) << 8;


            for ($bitwise = 0; $bitwise < 8; $bitwise++) {
                if (($result <<= 1) & 0x10000){
                    $result ^= $CRC_16_ARC_->Poly;
                }
                $result &= 0xFFFF;
            }
        }
        return bindec(strrev(sprintf( "%016d", decbin($result))));
    }
}
